...
pub struct TDigest<T, const MAX_CLUSTERS: usize = 1000> {
    pub means: Vec<T>,
    // Store weights internally as u32 for performance, expose as NonZeroU32 at API level
    weights: Vec<u32>,
    delta: Delta<T>,
}
...
{
    /// Get access to the means (centroids)
    pub fn means(&self) -> &[T] {
        &self.means
    }

    /// Get weights as NonZeroU32 iterator for API safety
    ...
    /// Get the delta parameter used for clustering
...
mod tests {
    ...
    fn test_tdigest_from_array_single_value() {
        ...
        // Weights are now internally u32, check through public interface
    ...
    fn test_tdigest_const_generic_default() {
        ...
        // Should use default MAX_CLUSTERS = 1000
        assert!(!digest.means.is_empty());
    }
    ...
    fn test_quantile_basic() {
        ...
        // These should be approximately correct for uniform data
    ...
    fn test_quantile_extremes() {
        ...
        // Should return values near the extremes
        assert!(q0 <= 2.0);
        assert!(q1 >= 4.0);
    }
    ...
    fn test_trimmed_mean() {
        ...
        // Trim 10% from each end (remove min and max)
    ...
    fn test_trimmed_mean_no_trim() {
        ...
        // No trimming (0% to 100%)
        let mean = digest.trimmed_mean(0.0, 1.0).unwrap();

        // Should be close to the actual mean (3.0)
    ...
    fn test_merge_basic() {
        ...
        // The merged digest should contain information from both
    ...
    fn test_merge_same_digest() {
        ...
        // Should have roughly the same statistical properties but doubled weights
        let original_median = digest.median().unwrap();
        let merged_median = merged.median().unwrap();

        // Medians should be approximately equal
    ...
    fn test_merge_one_empty() {
        ...
        // Both should be similar to the original non-empty digest
    ...
    fn test_merge_large_datasets() {
        ...
        // Test that merged quantiles are reasonable
    ...
    fn test_trimmed_mean_edge_cases() {
        ...
        // Test full range (should be like regular mean)
    ...
    fn test_very_small_dataset() {
        ...
        // All operations should work with single value
        let q25 = digest.quantile(0.25).unwrap();
        let median = digest.median().unwrap();
        let q75 = digest.quantile(0.75).unwrap();
        let trimmed = digest.trimmed_mean(0.1, 0.9).unwrap();
    ...
    fn test_large_range_values() {
        ...
        // Should handle very different scales
        let median = digest.median().unwrap();
        assert!(median.is_finite());
        assert!(median > 0.0);
    }
    ...
    fn test_f32_precision() {
        ...
        // Should work with f32 as well as f64
        let median = digest.median().unwrap();
        assert!(median >= 2.0 && median <= 4.0);
    }

    // T-Digest Literature-Aligned Tests

    #[test]
    fn test_quantile_accuracy_uniform_distribution() {
        // Test T-Digest accuracy on uniform distribution
        // Literature: T-Digest should have bounded error, especially for extreme quantiles
        let data: Vec<f64> = (1..=10000).map(|i| i as f64).collect();
        let delta = Delta::new(100.0).unwrap();
        let digest: TDigest<f64> = TDigest::from_array(&data, delta).unwrap();

        // Test extreme quantiles (where T-Digest should be most accurate)
        let q01 = digest.quantile(0.01).unwrap();
        let q99 = digest.quantile(0.99).unwrap();

        // For uniform distribution [1, 10000], q01 ≈ 100, q99 ≈ 9900
        let expected_q01 = 100.0;
        let expected_q99 = 9900.0;

        // T-Digest should have error < 1% for extreme quantiles with δ=100
    ...
    fn test_quantile_accuracy_normal_distribution() {
        // Test on normal-like distribution (using proper Box-Muller transform)
        let mut data = Vec::new();

        // Use a simple but effective LCG for better uniformity
        ...
        for _ in 0..5000 {  // Generate 5000 pairs = 10000 values
            ...
            // Ensure u1 is never exactly 0 to avoid ln(0)
        ...
        // For standard normal: median ≈ 0, q75-q25 ≈ 1.35
        let median = digest.median().unwrap();
        let q25 = digest.quantile(0.25).unwrap();
        let q75 = digest.quantile(0.75).unwrap();
        let iqr = q75 - q25;
    ...
    fn test_compression_effectiveness() {
        // T-Digest should compress large datasets into small number of centroids
        // Literature: number of centroids should be roughly proportional to δ, not input size
        ...
        // With δ=100, we should have roughly 100-300 centroids regardless of input size
    ...
    fn test_merge_associativity() {
        // T-Digest merge should be roughly associative: (A⊕B)⊕C ≈ A⊕(B⊕C)
        // This is important for streaming and distributed scenarios
        ...
        // Compare key quantiles - they should be similar (within reasonable tolerance)
    ...
    fn test_delta_effect_on_accuracy() {
        // Smaller δ should give more accuracy (more centroids) but less compression
        // Larger δ should give less accuracy (fewer centroids) but better compression
        ...
        // Based on our corrected understanding of the scale function:
        // Smaller delta creates larger q_limit steps, which means stricter clustering (fewer centroids)
        // Larger delta creates smaller q_limit steps, which means looser clustering (more centroids)
        // This is actually consistent with the literature if we understand "compression parameter" correctly
        ...
        // Both should give reasonable quantile estimates, but precise should be more accurate
        ...
        // This is probabilistic, but generally precise should be better
    ...
    fn test_monotonicity_property() {
        // T-Digest quantile function should be monotonic: q1 < q2 => Q(q1) ≤ Q(q2)
    ...
    fn test_performance_regression_large_dataset() {
        ...
        // Assert reasonable performance bounds (these will serve as regression tests)
        println!("Performance baseline: construction={:?}, query={:?}, multi_query={:?}",
                construction_time, query_time, multi_query_time);

        // Conservative bounds - should be much faster in practice
    ...
    fn test_memory_allocation_bounds() {
        ...
        // Verify delta affects compression as expected
    ...
    fn test_numerical_stability_edge_cases() {
        // Test very large numbers, very small numbers, mixed scales
        let mut data = vec![1e-100_f64, 1.0, 1e100];
        // Add some normal-range values
        ...
        // Test that extreme values don't break the algorithm
    ...
    fn test_merge_performance_scaling() {
        // Test that merge performance scales reasonably
        ...
        for &size in &sizes {
            ...
            // Merge should be fast regardless of input size (since it operates on centroids, not raw data)
    ...
    fn test_internal_weight_consistency() {
        // This test will help ensure our weight storage optimization doesn't break anything
        let means = vec![1.0_f64, 2.0, 3.0];
        let weights = vec![
            NonZeroU32::new(1).unwrap(),
        ...
        // Test quantile calculation consistency
        let q50 = digest.quantile(0.5).unwrap();
    ...
    fn test_vector_capacity_optimization() {
        // Test that our capacity optimizations work correctly
        let data: Vec<f64> = (0..1000).map(|i| i as f64).collect();
        let digest: TDigest<f64> = TDigest::from_array(&data, Delta::new(100.0).unwrap()).unwrap();

        // After clustering, we should have far fewer centroids than input points
    ...
    fn test_large_merge_memory_efficiency() {
        // Test memory efficiency with large merges
        ...
        for &size in &sizes {
            ...
            // Compression should be consistent regardless of input size
    ...
    fn test_empty_and_small_allocation_edge_cases() {
        ...
        // Test merging two small digests
    ...
    fn test_allocation_bounds_with_const_generics() {
        // Test the MAX_CLUSTERS constraint works properly
        let large_data: Vec<f64> = (0..10000).map(|i| i as f64).collect();

        // Create a digest with small max clusters
        ...
        match small_result {
            Ok(digest) => {
                ...
                // Should still provide reasonable quantile estimates
                let median = digest.median().unwrap();
                assert!(median >= 4000.0 && median <= 6000.0, "Median out of expected range: {}", median);
            }
            Err(e) => {
                // It's also acceptable for it to fail with cluster limit exceeded
                assert!(e.to_string().contains("exceed maximum"));
            }
        }
